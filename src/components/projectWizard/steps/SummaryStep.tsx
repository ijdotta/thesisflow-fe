...
import { SUBTYPE_LABELS } from '../types';
...
export function SummaryStep({ draft }: Props) {
  ...
  const localizedSubtypes = draft.subtypes.length > 0 
    ? draft.subtypes.map(st => SUBTYPE_LABELS[st] || st).join(', ')
    : '—';
  return (
    <div className="space-y-6 p-4">
      ...
      <div className="grid gap-3 text-sm">
        ...
        <div className="flex justify-between gap-4"><span className="font-medium">Subtipos</span><span>{localizedSubtypes}</span></div>
