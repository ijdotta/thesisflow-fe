...
// --- Draft Types ---
interface PersonBase { id?: string; name: string; lastname: string; email?: string; }
interface StudentDraft extends PersonBase { studentId?: string; career?: string; }
interface Domain { publicId: string; name: string; }
    ...
    <div className="flex flex-wrap gap-1">
      {list.map((p,i) => (
        <Badge key={i} variant="outline" className="gap-1 pr-1">
          {p.lastname}, {p.name}
          <button onClick={() => onRemove(i)} className="hover:bg-muted rounded p-0.5" aria-label="remove"><X className="h-3 w-3"/></button>
        </Badge>
      ))}
    </div>
  );
...
export default function CreateProjectWizard({ onCreated }: { onCreated?: () => void }) {
  ...
  // Debounces
  function useDebounce(value: string, delay=300) { const [v,setV]=React.useState(value); React.useEffect(()=>{ const t=setTimeout(()=>setV(value),delay); return ()=>clearTimeout(t); },[value,delay]); return v; }
  const dDir = useDebounce(dirQuery); const dCoDir = useDebounce(coDirQuery); const dCollab = useDebounce(collabQuery); const dStudent = useDebounce(studentQuery); const dDomain = useDebounce(domainQuery);

  // --- Effects (replace old ones) ---
  React.useEffect(()=>{ async function run(){ if(!dDir){setDirResults([]);return;} try{ const {data}=await api.get('/professors',{params:{q:dDir}}); setDirResults(data?.content||data||[]);}catch{}} run(); },[dDir]);
  React.useEffect(()=>{ async function run(){ if(!dCoDir){setCoDirResults([]);return;} try{ const {data}=await api.get('/professors',{params:{q:dCoDir}}); setCoDirResults(data?.content||data||[]);}catch{}} run(); },[dCoDir]);
  React.useEffect(()=>{ async function run(){ if(!dCollab){setCollabResults([]);return;} try{ const {data}=await api.get('/people',{params:{q:dCollab}}); setCollabResults(data?.content||data||[]);}catch{}} run(); },[dCollab]);
  React.useEffect(()=>{ async function run(){ if(!dStudent){setStudentResults([]);return;} try{ const {data}=await api.get('/students',{params:{q:dStudent}}); setStudentResults(data?.content||data||[]);}catch{}} run(); },[dStudent]);
  React.useEffect(()=>{ async function run(){ if(!dDomain){setDomainResults([]);return;} try{ const {data}=await api.get('/application-domains',{params:{q:dDomain}}); setDomainResults(data?.content||data||[]);}catch{}} run(); },[dDomain]);
  React.useEffect(()=>{ async function run(){ try{ const {data}=await api.get('/careers'); setCareerList(data?.content?.map((c:any)=>c.name)||data||[]);}catch{}} run(); },[]);
  ...
  function removeFrom(key: 'directors'|'codirectors'|'collaborators'|'students', index: number) {
    setDraft(d => { const arr = [...(d as any)[key]]; arr.splice(index,1); return { ...d, [key]: arr }; });
  }
  ...
  function parsePersonInput(input: string): PersonBase | null {
    ...
    if(trimmed.includes(',')) { const [last,name] = trimmed.split(',').map(s=>s.trim()); return { lastname: last || name || '', name: name || last || '' }; }
    const parts = trimmed.split(/\s+/); if(parts.length===1) return { name: parts[0], lastname: '' };
    const lastname = parts.pop()!; const name = parts.join(' '); return { name, lastname };
  }
  function addManual(key: 'directors'|'codirectors'|'collaborators', raw: string) {
    const person = parsePersonInput(raw); if(!person) return; addTo(key, person);
  }
        ...
        return (
          <div className="space-y-4 p-4">
            ...
            <div className="space-y-1">
              ...
              <Input value={draft.title} onChange={e=>updateDraft('title', e.target.value)} placeholder="Título del proyecto" />
            </div>
            <div className="grid sm:grid-cols-2 gap-4">
              ...
              <div className="space-y-1">
                ...
                <Input type="date" value={draft.initialSubmission || ''} onChange={e=>updateDraft('initialSubmission', e.target.value)} />
              </div>
            </div>
            ...
            <div className="space-y-1">
              ...
              {!!domainResults.length && (
                <div className="max-h-40 overflow-auto border rounded-md divide-y text-sm bg-background">
                  {domainResults.map(d => (
                    <button key={d.publicId} type="button" onClick={()=>{ updateDraft('applicationDomain', d); setDomainQuery(''); setDomainResults([]); }} className="w-full text-left px-2 py-1 hover:bg-accent">
                      {d.name}
                    </button>
                  ))}
                </div>
              ...
              {draft.applicationDomain && (
                <div className="text-xs flex items-center gap-2">Seleccionado: <Badge variant="secondary">{draft.applicationDomain.name}</Badge>
                  <button onClick={()=>updateDraft('applicationDomain', null)} className="text-muted-foreground hover:underline">Quitar</button></div>
              )}
            </div>
          </div>
        );
          ...
          <div className="space-y-8 p-4">
            ...
            <div className="space-y-2">
              ...
              {!!dirResults.length && <div className="border rounded-md max-h-36 overflow-auto divide-y text-sm mb-2">
                {dirResults.map(p => (
                  <button key={p.id || p.name+p.lastname} onClick={()=>{ addTo('directors', p); setDirQuery(''); setDirResults([]); }} className="w-full text-left px-2 py-1 hover:bg-accent">{[p.lastname,p.name].filter(Boolean).join(', ')}</button>
                ))}
              </div>}
            ...
            <div className="space-y-2">
              ...
              {!!coDirResults.length && <div className="border rounded-md max-h-36 overflow-auto divide-y text-sm mb-2">
                {coDirResults.map(p => (
                  <button key={p.id || p.name+p.lastname} onClick={()=>{ addTo('codirectors', p); setCoDirQuery(''); setCoDirResults([]); }} className="w-full text-left px-2 py-1 hover:bg-accent">{[p.lastname,p.name].filter(Boolean).join(', ')}</button>
                ))}
              </div>}
            ...
            <div className="space-y-2">
              ...
              {!!collabResults.length && <div className="border rounded-md max-h-36 overflow-auto divide-y text-sm mb-2">
                {collabResults.map(p => (
                  <button key={p.id || p.name+p.lastname} onClick={()=>{ addTo('collaborators', p); setCollabQuery(''); setCollabResults([]); }} className="w-full text-left px-2 py-1 hover:bg-accent">{[p.lastname,p.name].filter(Boolean).join(', ')}</button>
                ))}
              </div>}
        ...
        return (
          <div className="space-y-6 p-4">
            ...
            <div className="flex gap-2 mb-2">
              ...
              <Button type="button" variant="outline" disabled={!studentQuery.trim()} onClick={()=>{ const parsed=parsePersonInput(studentQuery); if(parsed) addTo('students',{...parsed}); setStudentQuery(''); }}>Añadir manual</Button>
            </div>
            {!!studentResults.length && <div className="border rounded-md max-h-40 overflow-auto divide-y text-sm mb-3">
              {studentResults.map(s => (
                <button key={s.id || s.name+s.lastname} onClick={()=>{ addTo('students', s); setStudentQuery(''); setStudentResults([]); }} className="w-full text-left px-2 py-1 hover:bg-accent">{[s.lastname,s.name].filter(Boolean).join(', ')}</button>
              ))}
            </div>}
              ...
              {draft.students.map((s,i) => (
                <div key={i} className="border rounded-md p-3 space-y-3">
                  <div className="flex gap-2 flex-wrap">
                    <Input className="h-8 flex-1 min-w-[120px]" placeholder="Nombre" value={s.name} onChange={e=>{ const v=e.target.value; setDraft(d=>{ const arr=[...d.students]; arr[i]={...arr[i], name:v}; return {...d, students:arr}; }); }}/>
                    <Input className="h-8 flex-1 min-w-[120px]" placeholder="Apellido" value={s.lastname} onChange={e=>{ const v=e.target.value; setDraft(d=>{ const arr=[...d.students]; arr[i]={...arr[i], lastname:v}; return {...d, students:arr}; }); }}/>
                    <Input className="h-8 w-40" placeholder="Legajo" value={s.studentId||''} onChange={e=>{ const v=e.target.value; setDraft(d=>{ const arr=[...d.students]; arr[i]={...arr[i], studentId:v}; return {...d, students:arr}; }); }}/>
                    <Select value={s.career||''} onValueChange={(val)=>{ setDraft(d=>{ const arr=[...d.students]; arr[i]={...arr[i], career:val}; return {...d, students:arr}; }); }}>
                    ...
                    <Button type="button" variant="ghost" className="h-8" onClick={()=>removeFrom('students', i)}><X className="h-4 w-4"/></Button>
                  </div>
                </div>
              ))}
            </div>
        ...
        return (
          <div className="space-y-6 p-4">
            ...
            <div className="grid gap-3 text-sm">
              ...
              <div className="flex justify-between gap-4"><span className="font-medium">Tipo</span><span>{draft.type || '—'}</span></div>
      ...
      <SheetContent side="right" className="sm:max-w-xl">
        <SheetHeader>
          ...
          <div className="flex gap-2 text-xs text-muted-foreground flex-wrap">
            {steps.map((s,i)=>(
              <div key={s} className={`px-2 py-1 rounded-md border ${i===step?'bg-primary text-primary-foreground border-primary':'bg-muted'}`}>{i+1}. {s}</div>
            ))}
          </div>
        </SheetHeader>
        ...
        <SheetFooter>
          <div className="flex justify-between gap-2 w-full">
            <div className="flex gap-2">
              ...
              <Button type="button" variant="ghost" disabled={saving} onClick={()=>{ reset(); setOpen(false); }}>Cancelar</Button>
            </div>
            <div className="flex gap-2">
              {step < steps.length-1 && (
